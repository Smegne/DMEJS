...
exports.getCourses = async (req, res) => {
    try {
        ...
        if (req.session.user) {
            ...
            console.log('✅ Fetched courses:', formattedCourses.length, 'Enrolled courses:', enrolledCourses, 'Applications:', applications, 'User ID:', req.session.user.id);
        } else {
            console.log('✅ Fetched courses:', formattedCourses.length, 'No user logged in');
        }
        res.render('courses', {
...
exports.approveApplication = async (req, res) => {
    try {
        ...
        const [result] = await pool.query('UPDATE course_applications SET status = ? WHERE id = ?', ['approved', id]);
        if (result.affectedRows === 0) {
